...
#include "keywords.h"
#include <math.h>
#define BB_VERSION_INFO "batari Basic v1.01dreveng38 (C)2005-2016\n"
...
//  if ((bank == 1) && (bs == 8)) newbank("2\n");
  ...
  printf(" if ECHOFIRST\n");
  if (bs==28) printf("       echo \"    \",[(DPC_graphics_end - *)]d , \"bytes of ROM space left");
  else printf("       echo \"    \",[(scoretable - *)]d , \"bytes of ROM space left");
  if (bs == 8) printf(" in bank 2");
  if (bs == 16) printf(" in bank 4");
  if (bs == 28) printf(" in graphics bank");
