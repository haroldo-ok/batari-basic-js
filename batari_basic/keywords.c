...
//printf("\n");
  ...
  while (1) {
    ...
    if (statement[1][0]=='\0') {return;}
    ...
    else if (!strncmp(statement[1],"bank\0",5)) newbank(atoi(statement[2]));
	     ...
	     (!strncmp(statement[3],"go\0",2))) ongoto(statement); // on ... goto or on ... gosub
    ...
    else if (!strncmp(statement[1],"pfpixel\0",8)) pfpixel(statement);
    else if (!strncmp(statement[1],"pfhline\0",8)) pfhline(statement);
    else if (!strncmp(statement[1],"pfclear\0",7)) pfclear(statement);
    else if (!strncmp(statement[1],"pfvline\0",8)) pfvline(statement);
    else if (!strncmp(statement[1],"pfscroll\0",9)) pfscroll(statement);
    ...
    else if (!strncmp(statement[1],"vblank\0",7)) vblank();
            ...
            || (!strncmp(statement[1],"player9color:\0",13))) player(statement);
    else if (!strncmp(statement[2],"=\0",1)) dolet(statement);
    else if (!strncmp(statement[1],"let\0",4)) dolet(statement);
